...
JOIN date_dim AS d1
  ON d1.d_date_sk = store_sales.ss_sold_date_sk AND d1.d_moy = 9 AND d1.d_year = 1999
JOIN item AS item
...
JOIN date_dim AS d2
  ...
  AND d2.d_year = 1999
JOIN date_dim AS d3
  ON catalog_sales.cs_sold_date_sk = d3.d_date_sk AND d3.d_year IN (1999, 2000, 2001)
GROUP BY
  item.i_item_id,
  item.i_item_desc,
  store.s_store_id,
