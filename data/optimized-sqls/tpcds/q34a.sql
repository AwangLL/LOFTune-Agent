WITH dn AS (
  ...
  JOIN date_dim AS date_dim
    ...
    AND (
      date_dim.d_dom < 30 OR date_dim.d_dom > 2
    )
    AND date_dim.d_year IN (1997, 1998, 2000)
  ...
  JOIN store AS store
    ON store.s_county IN ('Luce County', 'Richland County', 'Walker County', 'Williamson County')
