WITH tmp1 AS (
  ...
  WHERE
    (
      ...
      OR item.i_brand IN ('exportiunivamalg #9', 'scholaramalgamalg #14', 'scholaramalgamalg #7', 'scholaramalgamalg #9')
    )
    AND (
      item.i_brand IN ('amalgimporto #1', 'edu packscholar #1', 'exportiimporto #1', 'importoamalg #1')
    ...
    AND (
      item.i_brand IN ('amalgimporto #1', 'edu packscholar #1', 'exportiimporto #1', 'importoamalg #1')
      OR item.i_class IN ('personal', 'portable', 'refernece', 'self-help')
    )
    AND (
      item.i_brand IN ('exportiunivamalg #9', 'scholaramalgamalg #14', 'scholaramalgamalg #7', 'scholaramalgamalg #9')
      OR item.i_category IN ('Men', 'Music', 'Women')
    )
    AND (
      item.i_brand IN ('exportiunivamalg #9', 'scholaramalgamalg #14', 'scholaramalgamalg #7', 'scholaramalgamalg #9')
    ...
    AND (
      item.i_category IN ('Men', 'Music', 'Women')
      OR item.i_class IN ('personal', 'portable', 'refernece', 'self-help')
    )
    AND (
