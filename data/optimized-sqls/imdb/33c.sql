...
JOIN kind_type AS kt1
  ON kt1.id = t1.kind_id AND kt1.kind IN ('episode', 'tv series')
JOIN kind_type AS kt2
  ON kt2.id = t2.kind_id AND kt2.kind IN ('episode', 'tv series')
...
JOIN title AS t2
  ON t2.production_year <= 2010 AND t2.production_year >= 2000
WHERE
  cn1.country_code <> '[us]'