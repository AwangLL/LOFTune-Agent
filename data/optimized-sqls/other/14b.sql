...
JOIN keyword AS k
  ON k.id = mk.keyword_id AND k.keyword IN ('murder', 'murder-in-title')
...
JOIN movie_info_idx AS mi_idx
  ON mi_idx.info > '6.0' AND mi_idx.movie_id = mk.movie_id AND mi_idx.movie_id = t.id
JOIN movie_keyword AS mk
  ON mk.movie_id = t.id
JOIN title AS t
  ON t.production_year > 2010
  AND (
    t.title LIKE '%Mord%' OR t.title LIKE '%Murder%' OR t.title LIKE '%murder%'
  )
WHERE
  it1.info = 'countries'